...
layer and the $k$th neuron in the previous layer. Also, define $z^{(N)}_j =
    \sum_k
...
\begin{equation}
    \mathcal{L} =   \frac{1}{M} \sum_l^M p_E \left(\hat{E}^l- E^l \right)^2 +
    p_F
    \abs{\hat{\vec{F}}^l- \vec{F}^l}^2
\end{equation}

where $M$ is the number of examples in the minibatch; $\hat{E}^l$ and
$\hat{\vec{F}}^l$
are the predicted energies and total forces; $E^l$ and $\vec{F}^l$ are the DFT
energies
and forces; and $p_E$, $p_F$ are tunable prefactors useful to improve the
